  ...
  let navigate = useNavigate();
    return <div>
      <Formik      
        ...
        onSubmit={(values) => {
          const result = props.updateMyProfile(values);
          result.then(() =>{
            navigate('/profile/21738');
          })
        }}
      >
        {({ values,
          ...
          isSubmitting }) => (
            <form onSubmit={handleSubmit}>
              ...
              {Object.keys(props.profile.contacts).map(contact=> {
                return <div>
                  <Field className={s.contactField} type="input" name={"contacts." + contact} onChange={handleChange} onBlur={handleBlur} value={values.contacts[contact]} placeholder={contact}/> <br></br>
                  {errors["contacts." + contact] & touched["contacts." + contact] ? <span className={s.error}>{errors["contacts." + contact]}</span> : null}
                </div>
              })}
              <br></br>
